...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isLogin) {
      ...
      if (loggedInUser) {
        toast.success('登入成功！');
        navigate(['staff', 'admin'].includes(loggedInUser.role || '') ? '/admin' : ['groomer', 'caregiver'].includes(loggedInUser.role || '') ? '/workbench' : '/dashboard');
